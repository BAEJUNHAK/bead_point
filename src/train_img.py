...
def parse_args():
    ...
    ap.add_argument("--sigma", type=float, default=5.0, help="키포인트 가우시안 표준편차(px)")
...
class PersistConsole(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        line = (
            ...
            f"train_mae={self._fmt(m.get('train_mae_px'))} "
            f"train_pck={self._fmt(m.get('train_pck_px'))} | "
            f"val_loss={self._fmt(m.get('val_loss'))} "
            f"val_mae={self._fmt(m.get('val_mae_px'))} "
            f"val_pck={self._fmt(m.get('val_pck_px'))}"
        )
        if best is not None:
