            ...
            if use_constraint and pkl_path and os.path.exists(pkl_path):
                ...
                if polyline_mask is not None:
                    ...
                    if mask_ratio > 0.01 or force_constraint:  # 마스크가 충분하거나 강제 적용
                        ...
                        masked_logits[mask_expanded == 0] = -1e10  # 마스크 외부를 큰 음수로 (GPU 안정성 개선)
                        probs = torch.sigmoid(masked_logits)  # sigmoid(-1e10) ≈ 0
                        
                        constraint_type = "강제 적용" if force_constraint and mask_ratio <= 0.01 else "정상 적용"
                        print(f"🎯 폴리라인 제약조건 {constraint_type} (마스크 외부 완전 차단)")
                        
                        # 🔍 디버깅: 마스킹 후 확률 분포 확인
                        nonzero_ratio = (probs > 1e-6).float().mean().item()
                        max_prob = probs.max().item()
                        min_prob = probs.min().item()
                        mask_area_prob = probs[polyline_mask.unsqueeze(1).expand_as(probs) > 0].mean().item() if (polyline_mask > 0).any() else 0
